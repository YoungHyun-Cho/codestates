...
export const Tab = () => {
  ...
  const [isCurrentMenu, setIsCurrentMenu] = useState(0);

  const menuArr = [
    { name: 'Tab1', content: 'Tab menu ONE', focused: true },
    { name: 'Tab2', content: 'Tab menu TWO', focused: false },
    { name: 'Tab3', content: 'Tab menu THREE', focused: false },
  ];
      ...
      <div>
        <TabMenu>
          ...
          {menuArr.map((item, i) => {
            return (
              <li 
                className={isCurrentMenu === i ? "submenu focused" : "submenu"}
        ...
        <Desc>
          ...
          <p>{menuArr[isCurrentMenu].content}</p>
        </Desc>
      </div>
    </>
  );
