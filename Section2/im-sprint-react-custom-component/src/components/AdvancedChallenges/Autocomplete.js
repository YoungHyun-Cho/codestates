...
export const Autocomplete = () => {
  ...
  const handleInputChange = (event) => {
     ...
     */
     ...
     if (inputValue) setHasText(true);
    const selectedOptions = deselectedOptions.map(item => {
      // for (let i = 0; i < inputLength; i++)
      //   if (item[i] === event.target.value[i]) setCount(count + 1);
      if (item.slice(0, event.target.value.length) === event.target.value) return item;
      else return undefined;
    });
    const filtered = selectedOptions.filter(item => item !== undefined);
    setOptions(filtered);
  };
  ...
  const handleDropDownClick = (clickedOption) => {
     ...
     */
    setInputValue(clickedOption.target.firstChild.data);
    // setInputValue(clickedOption.target.textContent);
    console.dir(clickedOption.target);
  };
...
export const DropDown = ({ options, handleComboBox }) => {
  return (
    <DropDownContainer>
      ...
      {options.map((item, i) => {
        return (
          <li key={i} onClick={(event) => handleComboBox(event)}>{item}</li>
        );
      })}
    </DropDownContainer>
  );
