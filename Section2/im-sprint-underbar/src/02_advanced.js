...
_.includes = function (arr, target) {
  // TODO: 여기에 코드를 작성합니다.
	let answer = false;
	_.each(arr, item => item === target ? answer = true : answer);
	return answer;
};
...
_.every = function (arr, iteratee) {
  // TODO: 여기에 코드를 작성합니다.
	...
	if (iteratee === undefined) 
		_.each(arr, item => !item ? answer = false : answer);
	else 
		_.each(arr, item => !iteratee(item) ? answer = false : answer);
	return answer;
};
...
_.some = function (arr, iteratee) {
  // TODO: 여기에 코드를 작성합니다.
	...
	if (iteratee === undefined) 
		_.each(arr, item => item === true ? answer = true : answer);
	else 
		_.each(arr, item => iteratee(item) ? answer = true : answer);
	return answer;
};
...
_.zip = function (...rest) {
  // TODO: 여기에 코드를 작성합니다.
	let answer = [];
	let longestLength = 0;
	_.each(rest, item => item.length > longestLength ? longestLength = item.length : longestLength);
	for (let i = 0; i < longestLength; i++) {
		var arr = [];
		for (let j = 0; j < rest.length; j++) {
			arr.push(rest[j].shift());
		}
		answer.push(arr);
	}
	return answer;
...
_.zipStrict = function (...rest) {
  // TODO: 여기에 코드를 작성합니다.
	...
	_.each(rest, item => item.length < shortestLength ? shortestLength = item.length : shortestLength);
...
//                      // 첫 번째 배열에 'e'가 먼저 등장
_.intersection = function (...rest) {
  // TODO: 여기에 코드를 작성합니다.
	let answer = [];
	let firstLength = rest[0].length;
	let temp;
	for (let i = 0; i < firstLength; i++) {
		_.each(rest, function (item) {
			temp = false;
			if (_.includes(item, rest[0][i])) temp = true;
		});
		if (temp) answer.push(rest[0][i]);
	}
