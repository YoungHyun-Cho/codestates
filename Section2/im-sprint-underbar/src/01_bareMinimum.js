...
_.filter = function (arr, test) {
  // TODO: 여기에 코드를 작성합니다.
	// arr을 순회하면서 test함수를 arr의 각 요소에 적용했을 때, 
	// 그 리턴값이 true인 요소만 새로운 배열에 담아 리턴
	let answer = [];
	_.each(arr, item => test(item) ? answer.push(item) : answer);
	return answer;
};
...
_.reject = function (arr, test) {
  ...
  // TIP: 위에서 구현한 `filter` 함수를 사용해서 `reject` 함수를 구현해 보세요.
	let answer = [];
	_.filter(arr, item => test(item) ? answer : answer.push(item));
	return answer;
};
...
_.uniq = function (arr) {
  // TODO: 여기에 코드를 작성합니다.
	let answer = [];
	_.filter(arr, item => _.indexOf(answer, item) === -1 ? answer.push(item) : answer);
	return answer;
};
...
_.map = function (arr, iteratee) {
  ...
  // _.each 함수와 비슷하게 동작하지만, 각 요소에 iteratee를 적용한 결과를 리턴합니다.
	let answer = [];
	_.each(arr, item => answer.push(iteratee(item)));
	return answer;
};
...
_.pluck = function (arr, keyOrIdx) {
  ...
  // TODO: 여기에 코드를 작성합니다.
	let answer = [];
	_.map(arr, item => answer.push(item[keyOrIdx]));
	return answer;
};
  ...
  // TODO: 여기에 코드를 작성합니다.
	if (initVal === undefined) {
		...
		for (let i = 1; i < arr.length; i++)
			initArr.push(arr[i]);
		_.each(initArr, item => initVal = iteratee(initVal, item, 1, arr));
	}
	else _.each(arr, item => initVal = iteratee(initVal, item, 0, arr));
	return initVal;
};
