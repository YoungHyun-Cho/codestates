...
_.delay = function (func, wait, ...rest) {
  // TODO: 여기에 코드를 작성합니다.
  return setTimeout(func, wait, ...rest);
};
...
_.includes = function (arr, target) {
  // TODO: 여기에 코드를 작성합니다.
  return _.indexOf(arr, target) === -1 ? false : true;
};
...
_.every = function (arr, iteratee) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (iteratee && !iteratee(arr[i])) return false;
...
_.some = function (arr, iteratee) {
  // TODO: 여기에 코드를 작성합니다.
  for (let i = 0; i < arr.length; i++) {
    if (iteratee && iteratee(arr[i])) return true;
...
_.extend = function (dest, ...src) {
  // TODO: 여기에 코드를 작성합니다.
  if (Object.keys(src[0]).length)
    _.each(src, item => dest = { ...dest, ...item });
  return dest;
};

// _.defaults는 _.extend와 비슷하게 동작하지만, 이미 존재하는 속성(key)을 덮어쓰지 않습니다.
_.defaults = function (dest, ...src) {
  // TODO: 여기에 코드를 작성합니다.
  if (Object.keys(src[0]).length)
    _.each(src, item => {
      for (let key in item)
        if (!dest.hasOwnProperty(key)) dest[key] = item[key];
    });
  return dest;
};
...
_.zip = function (...arrs) {
  // TODO: 여기에 코드를 작성합니다.
  let answer = [];
  let maxLength = Math.max(..._.map(arrs, item => item.length));
  for (let i = 0; i < maxLength; i++) {
    answer.push([]);
    _.each(arrs, item => answer[i].push(item[i]));
  }
  return answer;
};
...
_.zipStrict = function (...arrs) {
  // TODO: 여기에 코드를 작성합니다.
  let answer = [];
  let maxLength = Math.min(..._.map(arrs, item => item.length));
  for (let i = 0; i < maxLength; i++) {
    answer.push([]);
    _.each(arrs, item => answer[i].push(item[i]));
  }
...
_.intersection = function (...arrs) {
  ...
  let standard = arrs[0];
  let answer = [];
  _.each(standard, item => {
    for (let i = 1; i < arrs.length; i++) {
      if (!_.includes(arrs[i], item)) break;
      if (i === arrs.length - 1) answer.push(item);
    }
  });
  return answer;
};
...
_.sortBy = function (arr, transform, order) {
  // TODO: 여기에 코드를 작성합니다.
  // arr의 각 요소를 transform한 결과를 order에 따라 정렬해라
  // underbar advanced sortBy에서 왜 정렬이 올바르게 되지 않는지 궁금합니다.
  let copy = _.map(arr, item => _.identity(item));
  if (transform) {
    if (order === -1) {
      copy.sort((a, b) => {
