...
_.take = function (arr, n) {
  // TODO: 여기에 코드를 작성합니다.
  return n === undefined ? [] : _.slice(arr, 0, n);
};
...
_.reject = function (arr, test) {
  ...
  let answer = [];
  _.filter(arr, item => !test(item) ? answer.push(item) : answer);
  return answer;
};
...
_.uniq = function (arr) {
  // TODO: 여기에 코드를 작성합니다.
  let answer = [];
  _.each(arr, item => _.indexOf(answer, item) === -1 ? answer.push(item) : answer);
  return answer;
};
...
_.reduce = function (arr, iteratee, initVal) {
  ...
  _.each(arr, item => {
    if (initVal === undefined) {
      _.indexOf(arr, item) !== 0 ? acc = iteratee(acc, item, 1, arr) : acc;
    }
    else {
      acc = iteratee(acc, item, 0, arr);
    }
  });
  return acc;
};