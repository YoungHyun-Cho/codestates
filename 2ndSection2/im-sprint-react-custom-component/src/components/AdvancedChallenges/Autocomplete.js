...
export const Autocomplete = () => {
  ...
  const handleInputChange = (event) => {
    ...
    setHasText(true);
    const selected = deselectedOptions.filter(item => item.slice(0, event.target.value.length) === event.target.value);
    setOptions(selected);
  };

  const handleDropDownClick = (clickedOption, e) => {
    /**
    ...
    setOptions(options.filter(item => item === clickedOption));
  };
  ...
  const handleKeyUp = (keyCode) => {
    if (keyCode === 40 && index < options.length - 1) setIndex(index + 1);
    else if (keyCode === 38 && index > 0) setIndex(index - 1);
    ...
    <div className='autocomplete-wrapper'>
      <InputContainer>
        <input 
          ...
          onKeyUp={(e) => e.keyCode === 38 || e.keyCode === 40 ? handleKeyUp(e.keyCode) : null}
        />
      ...
      {inputValue ? <DropDown options={options} handleComboBox={handleDropDownClick} /> : null}
    </div>
  );
};
...
export const DropDown = ({ options, handleComboBox }) => {
  return (
    <DropDownContainer >
      ...
      {options.map((item, index)=> <li onClick={() => handleComboBox(item)} key={index}>{item}</li>)}
    </DropDownContainer>
  );
};