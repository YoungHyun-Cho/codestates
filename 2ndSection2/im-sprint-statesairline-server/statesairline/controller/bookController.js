...
module.exports = {
  ...
  findById: (req, res) => {
    ...
    if (req.query.flight_uuid) {
      data = booking.filter(item => item.flight_uuid === req.query.flight_uuid);
      return res.status(200).json(data);
    }
    if (req.query.phone) {
      data = booking.filter(item => item.phone === req.query.phone);
      return res.status(200).json(...data);
    }
  ...
  create: (req, res) => {
    ...
    return res.status(201).json(booking.length - 1);
  },
  ...
  deleteById: (req, res) => {
    // TODO:
    let data = booking.filter(item => item.phone !== req.query.phone);
    return res.status(200).json(data);
  }
};
