...
export const App = () => {
  const [movieList, setMovieList] = useState(mockMovie);
  const [currentMovie, setCurrentMovie] = useState();

  const handleCardClick = id => {
    console.log('영화 목록을 클릭했군요!');
    // TODO: 현재 선택한 영화가 바뀌어야 합니다
    setCurrentMovie(...movieList.filter(item => item.id === id));
  };
  
  useEffect(() => getMovies().then(json => setMovieList(json)), []);
  ...
  return (
    <>
      ...
      <div className='body'>
        ...
        <MovieRankList movies={movieList} handleCardClick={handleCardClick} />
      </div>
    </>
  );
};
