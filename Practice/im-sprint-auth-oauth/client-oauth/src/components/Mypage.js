...
class Mypage extends Component {
  ...
  async getGitHubUserInfo() {
    ...
    const { name, login, html_url, public_repos } = userInfo.data;
  ...
  async getImages() {
    ...
    const images_blob = res.data.images.map(item => item.blob);
    this.setState({ images: images_blob });
  }
      ...
      <div>
        <div className='mypageContainer'>
          ...
          <div id="images">
            ...
            {this.state.images.map((item, i) => <img src={item} key={i} />)}
          </div>
        </div>
      </div >
    );
