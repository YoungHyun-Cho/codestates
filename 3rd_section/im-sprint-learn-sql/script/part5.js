...
const PART5_2_7 = `select title, body, created_at from content 
    ...
    inner join category on category.id=content_category.categoryId
...
*/
const PART5_2_10 = `select user.name as name, count(content.title) as ContentCount from content right join user on user.id=content.userId group by user.id`;
